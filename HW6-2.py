...
LETTERS_LOW = list(''.join(LETTERS).lower())
ANY_LOW = anchor(LETTERS_LOW)

def is_letter(sq):
    return isinstance(sq, str) and (sq in LETTERS or sq in LETTERS_LOW)
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    if pre.upper() in PREFIXES:       
        ...
        if is_letter(sq):
            ...
            low_possibilities = sq if isinstance(sq, set) else ANY_LOW
            for L in hand:
                if L =='_':
...
def find_prefixes(hand, pre='', results=None):
    ...
    pre_upp = pre.upper()
    if pre_upp in WORDS or pre_upp in PREFIXES: results.add(pre)
    if pre_upp in PREFIXES:
        for L in hand:
            if L =='_':
                for l in LETTERS_LOW:
